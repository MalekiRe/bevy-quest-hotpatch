...
pub fn hot(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let hot_fn = quote! {
        ::quest_hotpatch::dioxus_devtools::subsecond::HotFn::current(#hotpatched_fn #maybe_generics)
    };
    ...
    let maybe_run_call = if rerun_on_hot_patch {
        quote! {
            #[allow(unused_braces)]
            let system = ::quest_hotpatch::__macros_internal::IntoSystem::<
                _,
                #output_or_unit,
                _,
            >::into_system(#original_fn_name #maybe_generics);
            let name = system.name();
            ::quest_hotpatch::__macros_internal::debug!("Hot-patched and rerunning system {name}");
            #hot_fn.call((world,))
        }
    } else {
        quote! {
            #[allow(unused_braces)]
            let name = ::quest_hotpatch::__macros_internal::IntoSystem::<
                _,
                #output_or_unit,
                _,
    ...
    let hotpatched_fn_definition = match has_single_world_param(sig) {
        WorldParam::Mut | WorldParam::Ref => quote! {
            #vis fn #hotpatched_fn #impl_generics(world: &mut ::quest_hotpatch::__macros_internal::World) #where_clause #original_output {
                if let Some(mut reload_positions) = world.get_resource_mut::<::quest_hotpatch::__macros_internal::__ReloadPositions>() {
        ...
        WorldParam::None => quote! {
            #vis fn #hotpatched_fn #impl_generics(world: &mut ::quest_hotpatch::__macros_internal::World) #where_clause #original_output {
                ...
                use ::quest_hotpatch::__macros_internal::SystemState;
    ...
    let result = quote! {
        ...
        #[cfg(debug_assertions)]
        #vis fn #original_fn_name #impl_generics(world: &mut ::quest_hotpatch::__macros_internal::World) #where_clause #original_output {
            use std::any::Any as _;
            let type_id = #hotpatched_fn #maybe_generics.type_id();
            let contains_system = world.get_resource::<::quest_hotpatch::__macros_internal::__HotPatchedSystems>().unwrap().0.contains_key(&type_id);
            if !contains_system {
                let hot_fn_ptr = #hot_fn.ptr_address();
                let system = move |world: &mut ::quest_hotpatch::__macros_internal::World| {
                    let needs_update = {
                        let mut hot_patched_systems = world.get_resource_mut::<::quest_hotpatch::__macros_internal::__HotPatchedSystems>().unwrap();
                ...
                world.resource_mut::<::quest_hotpatch::__macros_internal::Schedules>()
                  .add_systems(
                    ::quest_hotpatch::__macros_internal::PreUpdate,
                    system.in_set(::quest_hotpatch::SimpleSubsecondSystemSet::UpdateFunctionPtrs)
                );
                let system = ::quest_hotpatch::__macros_internal::__HotPatchedSystem {
                    current_ptr: hot_fn_ptr,
                    last_ptr: hot_fn_ptr,
                };
                world.get_resource_mut::<::quest_hotpatch::__macros_internal::__HotPatchedSystems>().unwrap().0.insert(type_id, system);
            }
...
pub fn derive_hot_patch_migrate(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl ::quest_hotpatch::migration::HotPatchMigrate for #name {
            fn current_type_id() -> ::core::any::TypeId {
                ::quest_hotpatch::dioxus_devtools::subsecond::HotFn::current(|| ::core::any::TypeId::of::<Self>()).call(())
            }
        }
    };
